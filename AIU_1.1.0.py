...
file_types = {
        '3D_objects': ['.fbx', '.dae'],
        # '.blend' removed because there is and additional nested folder that requires additional coding
        ...
        # .blend works seamlessly but can also be used for materials
...
def process_3d_objects(file_path, textures_path, imported_assets_path):
    ...
    if file_extension in file_types['3D_objects']:
        ...
        read_and_rename(os.path.basename(file_path), os.path.join(os.path.dirname(file_path), "textures"))
    
        # Move object file to the 'Imported Assets' folder
        fbx_path = file_path
        destination_path = os.path.join(imported_assets_path, os.path.basename(file_path))
        ...
        directory = os.path.join(os.path.dirname(file_path), f"{os.path.basename(file_path).split('.')[0]}_txtr")
        if os.path.isdir(directory):
            source_path = directory
            destination_path = os.path.join(textures_path, os.path.basename(directory))
...
def sort_files(file_paths, unity_project_path, textures_path, imported_assets_path):
    # Process each file individually
...
def process_file(zip_files, unity_project_path, progress_var, is_running_var):
    try:
        ...
        for zip_path in zip_files:
            ...
            extract_path = os.path.join(os.path.dirname(zip_path), f"temp_extraction_{os.path.basename(zip_path)[:-4]}")
            ...
            sort_files([os.path.join(extract_path, file) for file in os.listdir(extract_path)], unity_project_path, textures_path, imported_assets_path)
        ...
        log_path = os.path.join(os.path.dirname(zip_path), log_filename)
...
def browse_button():
    ...
    entry_path.insert(0, ";".join(file_selected))
...
def run_button():
    if is_running_var.get():
        ...
        zip_files = entry_path.get().split(";")
        unity_project_path = entry_unity_path.get()
        if all(os.path.exists(file) for file in zip_files) and os.path.exists(unity_project_path):
            progress_var.set(0)  # Reset progress bar
            Thread(target=process_file, args=(zip_files, unity_project_path, progress_var, is_running_var)).start()
        else:
            result_label.config(text="Error: Invalid file or directory paths.")
